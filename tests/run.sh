...
credential_definitions() {
  ...
  CRED_DEF_ID=`cargo run -q -- -e=$ENDPOINT credential-definitions create --schema-id=$SCHEMA_ID 2> /dev/null`
...
credentials() {
  echo "--- Credentials ----"
  SCHEMA_ID=`cargo run -q -- -e=$ENDPOINT schemas create -n=foo -a=bar -a=baz 2> /dev/null 2> /dev/null`
  CRED_DEF_ID=`cargo run -q -- -e=$ENDPOINT  credential-definitions create --schema-id=$SCHEMA_ID 2> /dev/null`
